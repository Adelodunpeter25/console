...
import { parseCredential, saveCredential } from "./token-store.js";
import {
  OAUTH_AUTH_URL,
  OAUTH_TOKEN_URL,
  USERINFO_URL,
...
function getGeminiCliUserAgent(modelId = "gemini-2.5-pro"): string {
  const platform = process.platform === "win32" ? "win32" : process.platform;
  const arch = process.arch === "x64" ? "x64" : process.arch;
  return `GeminiCLI/${DEFAULT_GEMINI_CLI_VERSION}/${modelId} (${platform}; ${arch}; terminal)`;
}

function getGeminiCliHeaders(modelId?: string) {
  return {
...
function getAntigravityUserAgent() {
  const version = DEFAULT_ANTIGRAVITY_VERSION;
...
async function reportDebugEvent(
  ...
  try {
    const envText = await import("node:fs/promises").then(fs =>
      fs.readFile("/Users/techclub/Documents/projects/console/.dbg/gemini-onboard-poll.env", "utf8")
    );
    const serverUrl = envText.match(/DEBUG_SERVER_URL=(.+)/)?.[1]?.trim() ?? "http://127.0.0.1:7777/event";
...
async function loadCodeAssist(
  ...
  const baseEndpoint = ideType === "ANTIGRAVITY"
  ...
  const headers: Record<string, string> = {
    ...
    ...(ideType === "ANTIGRAVITY" ? { "User-Agent": getAntigravityUserAgent() } : getGeminiCliHeaders()),
  };
  ...
  const response = await fetch(endpoint, {
    ...
    headers,
    body: JSON.stringify({
      ...(ideType === "GEMINI_CLI" && envProjectId ? { cloudaicompanionProject: envProjectId } : {}),
      metadata: {
  ...
  await reportDebugEvent("B", "login.ts:loadCodeAssist", "[DEBUG] loadCodeAssist response received", {
    ideType,
  ...
  await reportDebugEvent("C", "login.ts:loadCodeAssist", "[DEBUG] loadCodeAssist payload parsed", {
    ...
    hasProjectIdObject: typeof data.cloudaicompanionProject === "object" && Boolean(data.cloudaicompanionProject?.id),
  });
  // #endregion
  ...
  if (ideType === "GEMINI_CLI" && tierId !== TIER_FREE && !envProjectId) {
    throw new Error(
      "This account requires setting the GOOGLE_CLOUD_PROJECT or GOOGLE_CLOUD_PROJECT_ID environment variable. " +
      "See https://goo.gle/gemini-cli-auth-docs#workspace-gca",
    );
...
async function loginWithConfig(config: OAuthConfig): Promise<void> {
  ...
  const { code, state: receivedState, close } = await startCallbackServer(config.port, config.callbackPath);
  close();
