...
const PANEL_FIELD_CONTEXT: &str = "ConsoleMenu > ComposerInput";
...
use crate::primitives::{IconName, app_icon, icon};
...
pub enum MenuItem {
    Entry {
        ...
        disabled: bool,
        /// Optional keyboard shortcut displayed at the trailing edge.
        shortcut: Option<SharedString>,
        /// Use the destructive theme color while keeping the item interactive.
        destructive: bool,
...
impl MenuItem {
    ...
    /// Display a keyboard shortcut beside this menu item.
    pub fn shortcut(mut self, value: impl Into<SharedString>) -> Self {
    ...
    /// Mark this menu item as destructive.
...
struct MenuCard {
    ...
    /// Context menus use the larger, native-looking surface from the desktop
    /// design; dropdowns keep the compact picker geometry.
    context_style: bool,
}
...
impl RenderOnce for MenuCard {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let mut card = div()
            ...
            .min_w(px(if context_style { 200.0 } else { 176.0 }))
            .max_w(px(if context_style { 300.0 } else { 320.0 }))
            .py(px(if context_style { 4.0 } else { 4.0 }))
            .rounded(px(if context_style { 10.0 } else { 9.0 }))
            .border_1()
        ...
        for (index, item) in items.into_iter().enumerate() {
            card = card.child(match item {
                MenuItem::Separator => div()
                    .my(px(if context_style { 6.0 } else { 4.0 }))
                    .mx(px(if context_style { 0.0 } else { 6.0 }))
                    .h(px(1.0))
                    .bg(theme.border)
                    .into_any_element(),
                ...
                MenuItem::Entry {
                    ...
                    .when_some(shortcut, |element, shortcut| {
                        element.child(
                            div()
                                .flex_none()
                                .text_size(px(if context_style { 13.0 } else { 10.0 }))
                                .text_color(theme.text_tertiary)
                                .child(shortcut),
                        )
                    })
                    .when(selected, |element| {
                        element.child(app_icon(IconName::Check, 11.0, theme.text_tertiary))
                    })
                    .into_any_element()
                }
...
fn row(
    ...
    div()
        .id(index)
        .mx(px(4.0))
        .px(px(if context_style { 12.0 } else { 8.0 }))
        .min_h(px(if context_style { 32.0 } else { 26.0 }))
        .rounded(px(if context_style { 7.0 } else { 6.0 }))
        .flex()
        .items_center()
        .gap(px(8.0))
        .text_size(px(if context_style { 13.0 } else { 11.5 }))
        .line_height(px(15.0))
