...
impl PromptHistory {
    ...
    fn record(&mut self, prompt: String) {
        if prompt.trim().is_empty() {
            return;
        }
        if self.entries.last() != Some(&prompt) {
    ...
    /// Move through history and return the content that should be displayed.
    /// `Some` means the arrow was consumed, including at a history boundary;
    /// `None` means there is no history navigation to perform.
    fn navigate(&mut self, down: bool, current: &str) -> Option<String> {
