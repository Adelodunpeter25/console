...
fn review_diff_gap_icon_path(direction: crate::review_diff::ExpansionDirection) -> &'static str {
    match direction {
        ...
        crate::review_diff::ExpansionDirection::Start => IconName::ChevronDown.path(),
        crate::review_diff::ExpansionDirection::End => IconName::ChevronUp.path(),
        crate::review_diff::ExpansionDirection::Both
        | crate::review_diff::ExpansionDirection::All => IconName::ChevronsUpDown.path(),
    }
}
...
impl RightPanelSurface {
    ...
    fn icon_path(&self) -> &'static str {
        match self {
            Self::Browser(_) => IconName::Globe.path(),
            Self::Terminal(_) => IconName::Terminal.path(),
            Self::BackgroundWork { key, .. } => work_kind_icon(key.kind),
            Self::Files => IconName::Folder.path(),
            Self::Diff => IconName::FileDiff.path(),
...
mod tests {
    ...
    fn review_gap_expansion_icons_match_pierre_visual_directions() {
        ...
        assert_eq!(
            ...
            IconName::ChevronUp.path()
        );
        assert_eq!(
            review_diff_gap_icon_path(ExpansionDirection::Both),
            IconName::ChevronsUpDown.path()
        );
    }
...
impl Waku {
    ...
    pub(super) fn render_right_panel_toggle(&self, cx: &mut Context<Self>) -> Stateful<Div> {
        ...
        div()
            ...
            .child(app_icon(IconName::PanelRight, 14.0, theme.text_tertiary))
        ...
        if !self.right_panel_surfaces.is_empty() {
            ...
            header = header.child(
                div()
                    ...
                    .child(dropdown_menu(
                        icon_button("add-right-panel-surface", IconName::Plus.path(), theme),
        ...
        div()
            ...
            .child(
                div()
                    ...
                    .child(app_icon(IconName::Folder, 13.0, theme.text_tertiary))
                    .child(
                        div()
                            .min_w_0()
                            .flex_1()
    ...
    fn render_right_panel_diff_toolbar(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        let refresh_icon: AnyElement = if self.right_panel_diff_loading {
            motion::spin(app_icon(IconName::LoaderCircle, 12.0, theme.text_tertiary))
        } else {
            app_icon(IconName::RotateCw, 12.0, theme.text_tertiary).into_any_element()
        };
        let refresh = div()
            ...
            .child(
                div()
                    ...
                    .child(
                        ...
                        .icon(IconName::Search.path(), 13.0)
                        .w_full(),
                    ),
            )
            .child(
