...
mod tests {
    ...
    fn session(status: Option<SessionStatus>) -> SessionHeader {
        SessionHeader {
            id: "session-1".into(),
            title: "Task".into(),
    ...
    #[test]
    fn explicit_waiting_state_wins_over_working_state() {
        let state = session_loading_state(
            &session(Some(SessionStatus::Working)),
            Some(100),
            true,
        );
        assert_eq!(state, Some(SidebarLoadingState::Waiting));
    }

    #[test]
    fn active_run_uses_local_start_time() {
        let state = session_loading_state(&session(None), Some(100), false);
        assert_eq!(
            state,
            Some(SidebarLoadingState::Working { started_at: 100 })
        );
    }

    #[test]
    fn restored_run_normalizes_millisecond_timestamp() {
        let state = session_loading_state(&session(Some(SessionStatus::Working)), None, false);
        assert_eq!(
            state,
            Some(SidebarLoadingState::Working {
