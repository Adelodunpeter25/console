...
impl Waku {
    ...
    pub(super) fn render_commit_dialog(&mut self, cx: &mut Context<Self>) -> Option<AnyElement> {
                ...
                .child(
                    div()
                        ...
                        .when(include_unstaged, |checkbox| {
                            checkbox.child(app_icon(IconName::Check, 12.0, theme.text))
                        }),
                )
                .child(
                    div()
        ...
        let commit = render_commit_action_row(
            ...
            &dialog.commit_focus,
            IconName::GitCommitHorizontal.path(),
            if commit_active {
                pending_status
                    .clone()
        ...
        let card = div()
            ...
            .child(
                div()
                    ...
                    .child(app_icon(IconName::GitBranch, 15.0, theme.text))
...
fn render_commit_action_row(
    ...
    let indicator = if active {
        motion::spin(app_icon(IconName::LoaderCircle, 15.0, theme.text_secondary))
    } else {
