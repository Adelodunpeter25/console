...
impl FsService {
    ...
    pub async fn pick_folder(&self) -> Result<String> {
        let url = self.transport.url("/api/fs/pick-folder").await;
        let resp = self
            .transport
            .client()
            .post(&url)
        ...
        let body: ApiResponse<PickFolderResponse> = resp
            .json()
            .await
            .context("Failed to parse pick folder response")?;
        if body.success {
            Ok(body.data.map(|d| d.path).unwrap_or_default())
        } else if body.cancelled == Some(true) {
            // The user dismissed the dialog — not an error. An empty path is
            // the caller's signal to do nothing.
            Ok(String::new())
        } else {
            Err(anyhow!(
                body.error.unwrap_or_else(|| "Failed to pick folder".into())
            ))
        }
    }

    /// Open a native file picker and return the selected file's path. An
    /// empty path means the user dismissed the dialog.
    pub async fn pick_file(&self) -> Result<String> {
        let url = self.transport.url("/api/fs/pick-file").await;
        let resp = self
            .transport
            .client()
            .post(&url)
        ...
        let body: ApiResponse<PickFileResponse> = resp
            .json()
            .await
            .context("Failed to parse pick file response")?;
        if body.success {
            Ok(body.data.map(|d| d.path).unwrap_or_default())
        } else if body.cancelled == Some(true) {
            Ok(String::new())
            ...
            Err(anyhow!(
                body.error.unwrap_or_else(|| "Failed to pick file".into())
            ))
        }
    }
}
