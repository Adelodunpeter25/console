// Inspired by @pierre/trees builtInIcons sprite sheets
...
export function resolveFileIconToken(fileName: string): BuiltInFileIconToken {
  const lower = fileName.toLowerCase();
  if (BUILT_IN_FILE_NAME_TOKENS[lower]) {
    return BUILT_IN_FILE_NAME_TOKENS[lower]!;
  }
  ...
  if (parts.length > 1) {
    for (let i = 1; i < parts.length; i++) {
      ...
      if (BUILT_IN_FILE_EXTENSION_TOKENS[ext]) {
        return BUILT_IN_FILE_EXTENSION_TOKENS[ext]!;
      }
    }
  }
