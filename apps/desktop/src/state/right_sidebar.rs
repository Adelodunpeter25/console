...
impl ConsoleDesktopApp {
    ...
    pub fn fetch_forwarded_ports(&mut self, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |_, cx| {
            let pid_opt = if pid_clone == "global" { None } else { Some(pid_clone.as_str()) };
    ...
    /// Manually forward a port on the server for the active workspace.
    pub fn forward_port(&mut self, port: u16, cx: &mut Context<Self>) {
        let project_id = self.active_project_id_or_global();
        let client = self.client.clone();
        let entity = cx.entity().downgrade();
        let pid_clone = project_id.clone();
        cx.spawn(async move |_, cx| {
            let pid_opt = if pid_clone == "global" { None } else { Some(pid_clone.as_str()) };
            if let Ok(new_port) = client.ports.forward(port, pid_opt).await {
                let _ = cx.update(|cx| {
                    if let Some(app) = entity.upgrade() {
                        app.update(cx, |this, cx| {
                            let list = this.forwarded_ports_by_project.entry(pid_clone).or_insert_with(|| Rc::new(Vec::new()));
                            let mut items = (**list).clone();
                            if !items.iter().any(|p| p.port == new_port.port) {
                                items.push(new_port);
                            }
                            this.forwarded_ports_by_project.insert(this.active_project_id_or_global(), Rc::new(items));
                            cx.notify();
                        });
                    }
                });
                    ...
                    if let Some(app) = entity.upgrade() {
                        app.update(cx, |this, cx| {
                            if let Some(list) = this.forwarded_ports_by_project.get_mut(&pid_clone) {
                                let items = list.iter().filter(|p| p.port != port).cloned().collect::<Vec<_>>();
                                *list = Rc::new(items);
                            }
                            cx.notify();
                        });
    ...
    pub fn open_port_in_browser(&mut self, url: String, window: &mut gpui::Window, cx: &mut Context<Self>) {
