...
@MainActor
final class WorkspaceSplitDragCoordinator: ObservableObject {
    struct ActiveDrag {
    ...
    @Published private var leafFrames: [UUID: OwnedFrame] = [:]
    /// Container-owned validation against the candidate tree after the
    /// source leaf has been removed and reinserted.
    @Published var canResolve: ((UUID, WorkspaceSplitDropResolution, CGSize) -> Bool)?
    @Published var onResolve: ((UUID, WorkspaceSplitDropResolution) -> Void)?
...
struct WorkspaceSplitDropZoneModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            ...
            .onChange(of: coordinator?.active?.sourceLeafId) { active in
