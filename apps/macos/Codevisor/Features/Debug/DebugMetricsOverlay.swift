...
@MainActor
final class DebugMetricsModel: ObservableObject {
    ...
    @Published private(set) var frameDurationMilliseconds = 0.0
    @Published private(set) var frameDurations: [Double] = []
    @Published private(set) var cpuPercent = 0.0
    @Published private(set) var memoryBytes: UInt64 = 0

    @Published private var frameTimestamps: [TimeInterval] = []
    @Published private var resourceTask: Task<Void, Never>?
    @Published private var previousResourceSample: ProcessResourceSample?
...
struct DebugMetricsOverlayModifier: ViewModifier {
    @StateObject private var model = DebugMetricsModel()
