...
export type ToolInputEventName = "tool_input_repaired" | "tool_input_invalid";

export interface ToolInputEvent {
  name: ToolInputEventName;
  /** Stable log/metric label, e.g. `tool_input_repaired:readFile`. */
  label: string;
  toolName: string;
  modelId?: string;
  provider?: string;
...
/** In-memory counters suitable for per-agent/session tool-call diagnostics. */
export class ToolInputTelemetry {
  private readonly counters = new Map<
    string,
    { modelId: string; provider: string; toolName: string; repaired: number; invalid: number }
  >();

  record(event: ToolInputEvent): void {
    const modelId = event.modelId ?? "unknown";
    const provider = event.provider ?? "unknown";
    const key = `${provider}:${modelId}:${event.toolName}`;
    ...
    if (event.name === "tool_input_repaired") counter.repaired++;
    else counter.invalid++;
    this.counters.set(key, counter);
  }

  snapshot(): ToolInputStats[] {
    return [...this.counters.values()].map((counter) => ({
      ...counter,
      repairRate:
        counter.repaired + counter.invalid === 0
          ? 0
          : counter.repaired / (counter.repaired + counter.invalid),
    }));
  }
