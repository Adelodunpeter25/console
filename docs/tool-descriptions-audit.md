...
| **writeFile** | `Write content to a file, fully replacing its contents… Parent directories are created automatically… Use for creating new files or completely rewriting… For targeted changes… use editFile instead…` | `Create or overwrite a file with new content. For targeted changes, use editFile instead.` | Keeps contrast with `editFile`; cross-reference retained per request. |
| **editFile** | `Edit a file by replacing one specific string… oldContent must match EXACTLY… Always call readFile first… If match appears 0 times or more than once… For complete rewrites, use writeFile.` | `Edit a file by replacing an exact string. Read the file first to get the exact match. For complete rewrites, use writeFile.` | Shortens but keeps exact-match, read-first guard, and cross-reference. |
